...
class COCOEvaluator:
    ...
    def _infer_on_single_gpu(gpu_id, chunk, model_args, question, cat_name_to_id):
        ...
        for i in tqdm(range(0, len(messages), args.batch_size)):
            ...
            batch_img_paths = [chunk["images"][i+j] for j in range(len(batch))]
            batch_img_dims = []
            for img_path in batch_img_paths:
                with Image.open(img_path) as img:
                    batch_img_dims.append((img.width, img.height))
            results.extend(COCOEvaluator._parse_output(outputs_decoded, batch_img_ids, cat_name_to_id, batch_img_dims))

        return results
    
    @staticmethod
    def _parse_output(outputs, img_ids, cat_name_to_id, img_dims):
        results = []
